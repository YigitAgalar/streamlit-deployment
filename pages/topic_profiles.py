...
grouped_topic_data = topic_data.groupby(["il","place_address","data_id","CustomName"]).mean().reset_index()
...
#sube = st.sidebar.selectbox("İncelemek istediğiniz şubenin adresini seçiniz", topic_data[topic_data["il"]==il]['place_address'].unique())

topic = st.sidebar.selectbox("İncelemek istediğiniz konuyu seçiniz", topic_data['CustomName'].unique())

threshold = st.sidebar.slider(f"{topic} konusunda seçilen pozitiflik oranının altındaki şubeleri getir ", 0, 100, 100)

sube=st.sidebar.selectbox("İncelemek istediğiniz şubenin adresini seçiniz",
    grouped_topic_data[(grouped_topic_data["il"]==il)&(grouped_topic_data["CustomName"]==topic)&(grouped_topic_data["review_label"]*100<threshold)]["place_address"].unique())
...
if sube is not None:
    ...
    st.error("Seçtiğiniz kriterlere uygun şube bulunamadı")